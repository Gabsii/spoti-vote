...
method.changePlaylist = async function(playlistId) {
    ...
    if (playlist) {
        ...
        if (playlist !== this.activePlaylist) {
            this.handler.log('INFO-[ROOM: '+this.id+']: Playlist changed to ['+playlist.name+'].');
        ...
        this.handler.log('[ERROR] Playlist: ' + playlistId + ' does not exist', 'error');
        return false;
    }
};
        ...
        if (playlist) {
            ...
            if (Array.isArray(playlist.tracks) === false) {
                if (playlist.tracks.total !== newPlaylists[i].tracks.total) {
                    ...
                    this.handler.log('INFO-[ROOM: '+this.id+']: Changed Playlist: ['+newPlaylists[i].name+'].');
                }
            } else {
            ...
            this.handler.log('INFO-[ROOM: '+this.id+']: Added new Playlist: ['+newPlaylists[i].name+'].');
        }
    }
    for (let i = 0; i < toBeRemoved.length; i++) {
        if (toBeRemoved[i]) {
            ...
            if (playlist.id !== this.activePlaylist.id) {
                ...
                this.handler.log('INFO-[ROOM: '+this.id+']: Deleted Playlist: ['+playlist.name+'].');
            }
        }
    }
    return true;
...
method.getRandomTracks = function(playlist, activeTrack) {
    ...
    this.handler.log('INFO-[ROOM: '+this.id+']: NewTracks: [' +selectedTracks[0].name+','+selectedTracks[1].name+','+selectedTracks[2].name+','+selectedTracks[3].name+ '] have been selected.');
...
method.refreshToken = async function() {
    this.handler.log('Before REFRESH:');
    this.handler.log('  - Access Token: ' + this.host.token);
    ...
    try {
        ...
        this.handler.log('ERROR-[ROOM: '+this.id+']: THERE WAS AN ERROR UPDATING THE TOKEN.\n' + e, 'error');
    }
    ...
    this.handler.log('After REFRESH:');
    this.handler.log('  - Access Token: ' + this.host.token);
    return true;
};
...
method.update = async function() {
    ...
    try {
        ...
        this.handler.log('ERROR-[ROOM: '+this.id+']: THERE WAS AN ERROR GETTING THE ACTIVE PLAYER.\n' + e, 'error');
    }
    ...
    if (this.activePlayer && this.activePlaylist) {
        if (this.activePlayer.timeLeft < 3000 && !this.isChanging) {
            ...
            this.handler.log('INFO-[ROOM: '+this.id+']: Reset Cooldown');
            this.isChanging = false;
        }
    }
    return true;
...
method.play = async function() {
    ...
    if (this.activePlaylist) {
        ...
        this.handler.log('INFO-[ROOM: '+this.id+']: ['+track.name+'] is now playing, since it had ['+track.votes+'] votes.');
...
method.reroll = async function() {
    if (this.activePlaylist) {
        ...
        this.handler.log('INFO-[ROOM: '+this.id+']: Reroll/NoReroll: ['+rerolls+'/'+((this.connectedUser.length+1)-rerolls)+'].');
        if (rerolls >= (2 * (this.connectedUser.length+1) / 3)) {
            this.handler.log('INFO-[ROOM: '+this.id+']: Rerolled.');
...
method.togglePlaystate = async function() {
    if (this.activePlayer) {
        if (this.activePlayer.isPlaying) {
            ...
            this.handler.log('INFO-[ROOM: '+this.id+']: Song is now Paused');
        } else {
            await fetch(this.host.spotifyApiAddress + '/v1/me/player/play',{
                headers: {
                    'Authorization': 'Bearer ' + this.host.token
            ...
            this.handler.log('INFO-[ROOM: '+this.id+']: Song is now Playing');
        }
        return true;
    }
};
